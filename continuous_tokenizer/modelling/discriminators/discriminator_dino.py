...
class SpectralNormFloat32(SpectralNorm):
    @staticmethod
    def apply(module, name, n_power_iterations, dim, eps):
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNormFloat32) and hook.name == name:
                raise RuntimeError("Cannot register two spectral_norm hooks on "
                                 "the same parameter {}".format(name))

        fn = SpectralNormFloat32(name, n_power_iterations, dim, eps)
    ...
    def __call__(self, module: nn.Module, inputs: Any) -> None:
        orig_dtype = getattr(module, self.name + '_orig').dtype
        with torch.autocast("cuda", enabled=False):
            # Make sure all weights are in float32
            setattr(module, self.name + '_orig', getattr(module, self.name + '_orig').float())
            setattr(module, self.name + '_u', getattr(module, self.name + '_u').float())
            setattr(module, self.name + '_v', getattr(module, self.name + '_v').float())
            super().__call__(module, inputs)

        setattr(module, self.name + '_orig', getattr(module, self.name + '_orig').to(orig_dtype))
        setattr(module, self.name + '_u', getattr(module, self.name + '_u').to(orig_dtype))
        setattr(module, self.name + '_v', getattr(module, self.name + '_v').to(orig_dtype))


class SyncBatchNormFloat32(nn.SyncBatchNorm):
