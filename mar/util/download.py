...
def download_pretrained_vae(overwrite=False):
    ...
    if not os.path.exists(download_path) or overwrite:
        ...
        r = safe_requests.get("https://www.dropbox.com/scl/fi/hhmuvaiacrarfg28qxhwz/kl16.ckpt?rlkey=l44xipsezc8atcffdp4q7mwmh&dl=0", stream=True, headers=headers)
...
def download_pretrained_marb(overwrite=False):
    ...
    if not os.path.exists(download_path) or overwrite:
        ...
        r = safe_requests.get("https://www.dropbox.com/scl/fi/f6dpuyjb7fudzxcyhvrhk/checkpoint-last.pth?rlkey=a6i4bo71vhfo4anp33n9ukujb&dl=0", stream=True, headers=headers)
...
def download_pretrained_marl(overwrite=False):
    ...
    if not os.path.exists(download_path) or overwrite:
        ...
        r = safe_requests.get("https://www.dropbox.com/scl/fi/pxacc5b2mrt3ifw4cah6k/checkpoint-last.pth?rlkey=m48ovo6g7ivcbosrbdaz0ehqt&dl=0", stream=True, headers=headers)
...
def download_pretrained_marh(overwrite=False):
    ...
    if not os.path.exists(download_path) or overwrite:
        ...
        r = safe_requests.get("https://www.dropbox.com/scl/fi/1qmfx6fpy3k7j9vcjjs3s/checkpoint-last.pth?rlkey=4lae281yzxb406atp32vzc83o&dl=0", stream=True, headers=headers)
